...
def select_action(node, temperature=1, deterministic=True):
    visit_counts = [(child.visit_count, action) for action, child in node.chideren.items()]
...
def make_results_dir(exp_path, args):
    ...
    if args.opr == 'train' and os.path.exists(exp_path) and os.listdir(exp_path):
        if not args.force:
            raise FileExistsError('{} is not empty. Please configure force to overwrite'.format(exp_path))
        else:
