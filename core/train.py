#TODO:
# * Utilize ray for parallel computation and faster training
# * Implement a logger to record data so we can tune hyperparameters later
# * Add a main function for the training, or a train()
# * Import test into this file for the _test helper function
...
from .config import BaseMuZeroConfig as MuZeroConfig
...
# Create the loggers for debug mode
...
def _log(config, step_count, log_data, model: Network, replay_buffer: ReplayBuffer, lr, worker_logs, summary_writer):
    # Unpackage all data into separate variables
    ...
    pred_reward, pred_value, target_policies, predicited_policies = td_data
    ...
    _msg = _msg.format(step_count, loss, weighted_loss, policy_loss, value_loss, 
                       reward_loss, replay_episodes_collected, replay_buffer_size, lr)
    ...
    if summary_writer is not None:
        if config.debug:
            for name, W in model.named_parameters():
                summary_writer.add_histogram('after_grad_clip' + '/' + name + '_grad', W.grad.data.cpu().numpy(), step_count)
        ...
        summary_writer.add_histogram('replay_data/target_reward', target_reward.flatten(), step_count)
        summary_writer.add_histogram('replay_data/target_value', target_value.flatten(), step_count)
        ...
        summary_writer.add_histogram('train_data_dist/transformed_target_value', trans_target_value.flatten(), step_count)
        summary_writer.add_histogram('train_data_dist/transformed_target_reward', trans_target_reward.flatten(), step_count)

        summary_writer.add_histogram('train_data_dist/target_reward_phi', target_reward_phi.unique().flatten(), step_count)
        summary_writer.add_histogram('train_data_dist/target_value_phi', target_value_phi.unique().flatten(), step_count)

        summary_writer.add_histogram('train_data_dist/pred_reward', pred_reward.flatten(), step_count)
        summary_writer.add_histogram('train_data_dist/pred_value', pred_reward.flatten(), step_count)

        summary_writer.add_histogram('train_data_dist/pred_policies', predicited_policies.flatten(), step_count)
        ...
        summary_writer.add_scalar('train/episodes_collected', ray.get(replay_buffer.episodes_collected.remote()), step_count)
...
# SharedStorage will be a storage of the model that we are currently training on, which many workers will pull from in parallel
# Slightly modified from the psuedocode to allow for logging and to work ray (parallel actors rather than sequential)
@ray.remote
class SharedStorage(object):
    ...
    def set_data_worker_logs(self, eps_len, eps_reward, temperature, visit_entropy):
        self.eps_length.append(eps_len)
    ...
    def get_worker_logs(self):
        if len(self.reward_log) > 0:
            ...
            eps_lengths = sum(self.eps_length) / len(self.eps_length)
        ...
        if len(self.test_log) > 0:
            test_scores = sum(self.test_log) / len(self.test_log)
            self.test_log = []
        else:
            test_scores = None

        return reward, eps_lengths, test_scores, temperature, visit_entropy 

# DataWorkers are actors who share the same network/storage and run multiple MCTS to collect training data quickly
# and efficiently
@ray.remote
    ...
    def run(self):
        ...
        with torch.no_grad():
            while ray.get(self.shared_storage.get_counter.remote()) < self.config.training_steps:
                # Setup network with current best weights and all other necessary variables
                ...
                # Run MCTS and current network until the game is either lost or max moves have been reached
                while not done and eps_steps < self.config.max_moves:
                    ...
                    root.add_exploration_noise(self.config.root_dirichlet_alpha, 
                                               self.config.exploration_faction)
...
def  update_weights(model, target_model, optimizer, replay_buffer: ReplayBuffer, config: MuZeroConfig):
    ...
    # Transfer tensors to device (config for GPU devices)
    ...
    target_reward_phi = config.reward_phi(target_reward)
    transformed_target_value = config.scalar_transform(target_value)
    target_value_phi = config.value_phi(target_value)

    # Get the initial reference values and transform into the categorical representation
    # breakpoint()
    value, _, policy_logits, hidden_state = model.initial_reference(obs_batch)
    scaled_value = config.inverse_value_transform(value)

    # Note: The following line is just for logging purposes
    ...
    for step_i in range(config.num_unroll_steps):
        # Compute the loss for all hidden states within the unroll
        ...
        reward_loss += config.scalar_reward_loss(reward, target_reward_phi[:, step_i + 1])
        hidden_state.register_hook(lambda grad: grad * 0.5)

        # Collected for logging 
        predicted_values = torch.cat((predicted_values, config.inverse_value_transform(value)))
        scaled_rewards = config.inverse_value_transform(reward)
...
def _train(config, shared_storage, replay_buffer, summary_writer):
    ...
    for step_count in range(config.training_steps):
        ...
        # update the target model based on some tau
...
def _test(config, shared_storage):
    ...
    while ray.get(shared_storage.get_counter.remote()) < config.training_steps:
        ...
        test_score = test(config, test_model, config.test_episdoes, 'cpu', False)
